    ...
    if len(features) == 1:
        ...
        if features[0].shape[3] != model.inputs[0].shape[3]:
            features_resampled = keras.layers.Conv2D(keras.backend.int_shape(model.inputs[0])[3], kernel_size=1,
                                                     ...
                                                     name=name+'feature_resampling')(features[0])
            return keras.layers.Lambda(lambda x: x, name=name)(model(features_resampled))
...
def retinanet_bbox(
    ...
    **kwargs
    ...
    # compute the anchors
    features = [model.get_layer(p_name).output for p_name in anchor_params.feature_layers]
